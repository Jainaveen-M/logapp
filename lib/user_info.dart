...
class _UserState extends State<User> {
  ...
  checkInPermission(){
    CollectionReference collectionReference = Firestore.instance.collection('permission');
    final dateTime = DateTime.now();
    Map<String,dynamic> info ={
      "check":1,
      "name":widget.user_name,
      "time":DateTimeFormat.format(dateTime, format: DateTimeFormats.american)+" IN",
      "img_url":widget.img_url,
    };
    collectionReference.document(widget.user_name).setData(info);
  }
  checkOutPermission(){
    CollectionReference collectionReference = Firestore.instance.collection('permission');
    final dateTime = DateTime.now();
    Map<String,dynamic> info ={
      "check":2,
      "name":widget.user_name,
      "time":DateTimeFormat.format(dateTime, format: DateTimeFormats.american)+" OUT",
      "img_url":widget.img_url,
    };
    collectionReference.document(widget.user_name).setData(info);
  }
  ...
  Future CheckPermissionStatus() async{
    QuerySnapshot qs= await Firestore.instance.collection('admin_permission').getDocuments();
    setState(() {
      check_Permission = qs.documents[0]['status'];
    });
  }
                                                  ...
                                                  children: [
                                                    TextField(
                                                      decoration: InputDecoration(
                                                        ...
                                                        labelText: 'Enter the task you completed',
                                                      ),
